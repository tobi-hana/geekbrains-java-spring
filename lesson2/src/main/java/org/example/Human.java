...
public class Human implements Moves {
    ...
    public void run(Treadmill treadmill) {
        ...
        if (treadmill.getLength() > maxRunLength){
            System.out.printf("Human '%s' can't run %.1f meters \n", name, treadmill.getLength());
            System.out.println();
            return;
        }
        thereAreObstacles = false;
        System.out.printf("Human '%s' successfully passed %.1f meters on this treadmill \n",
                name, treadmill.getLength());
    }
    ...
    public void jump(Wall wall) {
        ...
        thereAreObstacles = false;
        System.out.printf("Human '%s' successfully jumped over %.1f meter wall \n",
                name, wall.getHeight());
    }
